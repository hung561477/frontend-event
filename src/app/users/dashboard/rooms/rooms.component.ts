...
@Component({
  ...
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.sm ? 'large' : 'medium'">
      ...
      <event-room-selector (select)="select($event)"></event-room-selector>
      <event-player [collapsed]="isCollapsed() && breakpoint.width <= breakpoints.md"></event-player>
    </nb-card>
  `,
})
